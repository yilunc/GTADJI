...
public class Police extends AI {
    ...
    public void shootPlayer(Player p, SpriteBatch batch) {
    ...
    public void draw(SpriteBatch batch, Player p, float deltaTime) {
        ...
        if (shotNum == 1) {
            frame = Assets.pm4flash;
            batch.draw(frame, bounds.x, bounds.y, frame.getRegionWidth() / 2, frame.getRegionHeight() / 2, frame.getRegionWidth(), frame.getRegionHeight(), 0.45f, 2.5f, lastRot, true);
        }
    }
}
